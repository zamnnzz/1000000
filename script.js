...
// Firebase يُحمّل عند الحاجة من lazy-firebase.js بدل تحميل المكتبة قبل رسم الصفحة.
const db = window.ZAMN_FIREBASE.db;
const firebase = window.ZAMN_FIREBASE.firebaseCompat;
const { games, articles, faqItems } = window.ZAMN_DATA;
...
const startBackgroundFirebase = () => {
  registerSiteVisit().catch(console.error);
  startPresenceTracking();
};

if ("requestIdleCallback" in window) {
  window.requestIdleCallback(startBackgroundFirebase, { timeout: 3000 });
} else {
  window.setTimeout(startBackgroundFirebase, 1200);
}
