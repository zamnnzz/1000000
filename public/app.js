...
const SESSION_KEY='jawabMajhoolSessionV1'; const EMOJIS=['😎','🤠','🥷','🤓','👽','🤖','🦊','🐼','🐯','🐸']; let pendingSession=null,booted=false;
const QUESTIONS=[
...
function renderEmojiPicker(room=null){
  ...
  $('emojiPicker').innerHTML=EMOJIS.map((emoji,i)=>{
    const owner=claims[i];
    const mine=selectedEmojiIndex===i;
    const locked=!!owner&&owner!==uid;
    return `<button type="button" class="emojiChoice ${mine?'selected':''} ${locked?'taken':''}" data-emoji-index="${i}" ${locked?'disabled':''} aria-label="${locked?'الشخصية محجوزة':`اختيار ${emoji}`}">${emoji}</button>`;
  }).join('');
  $('emojiHint').textContent=selectedEmojiIndex===null?'اختر إيموجي واحد':`اختيارك: ${EMOJIS[selectedEmojiIndex]}`;
...
async function selectEmoji(i){
  ...
  if(!tx.committed)return toast('هذا الإيموجي اختاره لاعب قبلك');
