...
$('nextBtn').onclick=async()=>{if(!isHost)return;const current=Number(lastRoom?.questionIndex||0);if(current<QUESTIONS.length-1){await startRound(current+1)}else{await update(ref(db,`rooms/${roomCode}`),{phase:'final',phaseStartedAt:Date.now()})}};
