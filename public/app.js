...
const QUESTIONS=[
 {"text":"كم عدد سور القرآن الكريم؟","answer":"114 سورة","accepted":["114","١١٤","١١٤ سورة"]},
...
document.querySelectorAll('.questionCountBtn').forEach(btn=>{
  btn.onclick=async()=>{
    if(!isHost||!roomCode)return;
    const count=Number(btn.dataset.count);
    if(![5,10,15,20].includes(count))return;
    await update(ref(db,`rooms/${roomCode}`),{selectedQuestionCount:count});
  };
});
...
$('startBtn').onclick=async()=>{
  ...
  const order=shuffle(QUESTIONS.map((_,i)=>i)).slice(0,actual);
  await update(ref(db,`rooms/${roomCode}`),{roundQuestionOrder:order,questionIndex:0});
  if(actual<wanted)toast(`المتاح حالياً ${actual} أسئلة فقط — سيتم تشغيلها للتجربة`);
