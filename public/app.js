...
document.querySelectorAll('.doorBtn').forEach(btn=>{
  btn.onclick=async()=>{
    ...
    if(![1,2,3,4].includes(door))return;
...
$('startBtn').onclick=async()=>{
  ...
  const door=[1,2,3,4].includes(Number(lastRoom?.selectedDoor))?Number(lastRoom.selectedDoor):1;
