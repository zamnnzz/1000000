...
function show(id){screens.forEach(s=>$(s).classList.toggle('hidden',s!==id));$('leaveBtn').classList.toggle('hidden',id==='home'||id==='nameScreen'||id==='howToScreen')}
...
$('submitAnswerBtn').onclick=async(e)=>{e?.preventDefault();interactionGuardUntil=Date.now()+1200;$('submitAnswerBtn').blur();$('answerInput').blur();const text=$('answerInput').value.trim().slice(0,60);if(!text)return toast('اكتب إجابة');const q={answer:lastRoom.answerKey,accepted:lastRoom.accepted||[]};const isRight=correct(q,text);await set(ref(db,`rooms/${roomCode}/answers/${uid}`),{text,correct:isRight});await update(ref(db,`rooms/${roomCode}/players/${uid}`),{submitted:true});toast(isRight?'الإجابة الصحيحة لا تُحسب كفخ — ستدخل التصويت مع الجميع':'تم تثبيت إجابتك');await maybeBuildOptions()};
