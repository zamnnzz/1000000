...
const QUESTIONS=[
 ...
 {"text":"ما اسم هذه الشخصية في مسلسل باب الحارة؟","answer":"أبو غالب","accepted":["ابو غالب","أبوغالب","ابوغالب"]},
...
document.querySelectorAll('.doorBtn').forEach(btn=>{
  btn.onclick=async()=>{
    ...
    if(![1,2,3,4,5,6].includes(door))return;
...
$('startBtn').onclick=async()=>{
  ...
  const door=[1,2,3,4,5,6].includes(Number(lastRoom?.selectedDoor))?Number(lastRoom.selectedDoor):1;
