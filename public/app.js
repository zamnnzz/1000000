...
function openCreate(){joinMode='create';$('nameTitle').textContent='اسم المضيف';$('codeInput').classList.add('hidden');$('codeInput').readOnly=false;$('codeInput').disabled=false;$('codeInput').classList.remove('lockedCode');$('confirmJoinBtn').textContent='إنشاء الغرفة';$('nameInput').value='';show('nameScreen')}
function openJoin(){joinMode='join';$('nameTitle').textContent='دخول الجلسة';$('codeInput').classList.remove('hidden');$('confirmJoinBtn').textContent='دخول';$('nameInput').value='';const linkedRoom=(new URL(location.href).searchParams.get('room')||'').toUpperCase();$('codeInput').value=linkedRoom;$('codeInput').readOnly=!!linkedRoom;$('codeInput').disabled=!!linkedRoom;$('codeInput').classList.toggle('lockedCode',!!linkedRoom);$('codeInput').placeholder=linkedRoom?'رقم الجلسة':'رقم الجلسة';$('codeInput').setAttribute('aria-label','رقم الجلسة');show('nameScreen')}
