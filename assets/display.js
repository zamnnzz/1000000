...
function renderGame(g){
 ...
 if(activeHints.length){
   $("activeHintsList").innerHTML=activeHints.map((h,i)=>`<b>${i+1}. ${h}</b>`).join("");
 ...
 if(g.hintUsedEvent?.id&&g.hintUsedEvent.id!==lastHintUsed){
   ...
   $("hintUsedText").textContent=`${g.hintUsedEvent.number?`تلميح ${g.hintUsedEvent.number}: `:""}${g.hintUsedEvent.hint}`;
