...
const bank={
"سيارات":{question:"اذكر أشهر ماركات السيارات اليابانية",answers:[["تويوتا",10],["نيسان",9],["هوندا",8],["مازدا",7],["سوبارو",6],["ميتسوبيشي",5],["سوزوكي",4],["لكزس",3],["إنفينيتي",2],["أكيورا",1]]},
"كرة قدم":{question:"اذكر أندية كرة قدم أوروبية مشهورة",answers:[["ريال مدريد",10],["برشلونة",9],["مانشستر يونايتد",8],["ليفربول",7],["بايرن ميونخ",6],["مانشستر سيتي",5],["ميلان",4],["إنتر ميلان",3],["أرسنال",2],["باريس سان جيرمان",1]]},
"جغرافيا":{question:"اذكر دولاً عربية معروفة",answers:[["السعودية",10],["مصر",9],["الإمارات",8],["المغرب",7],["العراق",6],["الأردن",5],["الكويت",4],["قطر",3],["عُمان",2],["البحرين",1]]},
"ثقافة عامة":{question:"اذكر أشياء نستخدمها يومياً",answers:[["الجوال",10],["الماء",9],["السيارة",8],["الإنترنت",7],["الكهرباء",6],["المفتاح",5],["الساعة",4],["القلم",3],["الكرسي",2],["الكوب",1]]},
"السعودية":{question:"اذكر مدناً سعودية مشهورة",answers:[["الرياض",10],["جدة",9],["مكة",8],["المدينة",7],["الدمام",6],["الطائف",5],["أبها",4],["تبوك",3],["الخبر",2],["حائل",1]]},
"أكل وطبخ":{question:"اذكر أكلات شعبية عربية مشهورة",answers:[["كبسة",10],["مندي",9],["شاورما",8],["فلافل",7],["كشري",6],["مسخن",5],["مقلوبة",4],["كبة",3],["فتة",2],["محشي",1]]}
};
function getRound(cat){return bank[cat]||{question:`اذكر 10 أشياء مرتبطة بفئة ${cat}`,answers:Array.from({length:10},(_,i)=>[`إجابة ${i+1}`,10-i])}}
if(!session){$("badSession").classList.remove("hidden")}else{
 ...
 onValue(gameRef,async snap=>{
  ...
  state=g;$("connectionBadge").textContent=`متصل — ${session}`;$("connectionBadge").classList.add("ok");
...
async function startRound(g){
 ...
 await update(gameRef,{status:"game",currentRoundIndex:g.currentRoundIndex,currentCategory:pick.category,currentOwner:pick.owner,currentQuestion:rd.question,currentAnswers:rd.answers.map(([text,points],i)=>({text,points,index:i,revealed:false})),score1:g.score1||0,score2:g.score2||0,answerTurn:pick.owner,revealEvent:null,wrongEvent:null,updatedAt:Date.now()});
}
...
async function reveal(i){
 ...
 const pts=a[i].points,newTurn=state.answerTurn===1?2:1,newScore1=(state.score1||0)+(state.answerTurn===1?pts:0),newScore2=(state.score2||0)+(state.answerTurn===2?pts:0);
 await update(gameRef,{currentAnswers:a,score1:newScore1,score2:newScore2,answerTurn:newTurn,revealEvent:{id:Date.now(),text:a[i].text,points:pts},wrongEvent:null,updatedAt:Date.now()});
}
$("wrongBtn").onclick=async()=>await update(gameRef,{answerTurn:state.answerTurn===1?2:1,wrongEvent:{id:Date.now()},revealEvent:null,updatedAt:Date.now()});
